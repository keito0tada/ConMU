...
def pruning_model(model, px):
    print(
        "Apply Unstructured L1 Pruning Globally (all conv layers and all linear layers)"
    )
    parameters_to_prune = []
    for name, m in model.named_modules():
        if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
...
def setup_model_dataset(args):
    if args.dataset == "cifar10":
        ...
        train_full_loader, val_loader, test_loader = cifar10_dataloaders_no_val(
            batch_size=args.batch_size, data_dir=args.data, num_workers=args.workers
        )
        marked_loader, _, _ = cifar10_dataloaders_no_val(
            batch_size=args.batch_size, data_dir=args.data, num_workers=args.workers
        )
...
def split_train_to_forget_retain(
    marked_loader, forget_percentage, batch_size, seed_=42
):
    ...
    if args.class_wise != None:
        if (args.class_wise).isdigit() and int(args.class_wise) in list(label):
            forget_indices = [
                idx for idx, label in enumerate(label) if label == int(args.class_wise)
            ]
            forget_class_indices = [
                i
                for i in random.sample(
                    forget_indices, int(len(forget_indices) * forget_percentage)
                )
            ]
            ...
            retain_dataset = Subset(
                ...
                indices=[
                    ...
                    if i not in forget_class_indices
                ],
            )
        ...
        retain_dataset = Subset(
            whole_dataset, indices=[i for i in range(split_point, len(whole_dataset))]
        )
